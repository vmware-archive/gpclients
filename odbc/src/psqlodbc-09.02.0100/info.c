...
mylog("CONVERT_FUNCTIONS=" FORMAT_ULEN "\n", value);
		...
		case SQL_DBMS_VER:		/* ODBC 1.0 */
			...
			{
			  intres = sscanf( conn->pg_version
					 , "%*s %*s (Greenplum Database %d.%d.%d.%d"
					 , &gpmajor, &gpminor, &gpsubminor, &gpsubsubminor
					 );
			  sizegpver = sprintf(tmp, "%.2d.%.2d.%.4d ", gpmajor, gpminor, gpsubminor);
			  strncpy_null(&tmp[sizegpver], conn->pg_version, sizeof(tmp) - sizegpver);
			  p = tmp;
			}
			else
			{
