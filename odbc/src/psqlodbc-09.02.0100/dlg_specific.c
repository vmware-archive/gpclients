...
unfoldCXAttribute(ConnInfo *ci, const char *value)
{
	...
	ci->greenplum = (char)((flag & BIT_GREENPLUM) != 0);
}
BOOL
copyAttributes(ConnInfo *ci, const char *attribute, const char *value)
{
	...
	else if (stricmp(attribute, INI_GREENPLUM) == 0 || stricmp(attribute, ABBR_GREENPLUM) == 0)
		ci->greenplum = atoi(value);
...
getDSNdefaults(ConnInfo *ci)
{
	...
	if (ci->rollback_on_error < 0) {
		char    *ptr;
		const char *value = DEFAULT_PROTOCOL;
		...
		if (ptr)
		{
			if ('-' != *value)
			{
				strncpy(ci->protocol, value, ptr - value);
				ci->protocol[ptr - value] = '\0';
			}
			ci->rollback_on_error = atoi(ptr + 1);
			mylog("rollback_on_error=%d\n", ci->rollback_on_error);
		}
	...
	if(ci->greenplum < 0)
		ci->greenplum = DEFAULT_GREENPLUM;
