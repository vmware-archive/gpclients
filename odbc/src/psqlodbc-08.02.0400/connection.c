			  ...
			  SQLSMALLINT cbAuthStr)
{
	...
	qlog("conn = %p, %s(DSN='%s', UID='%s', PWD='%s')\n", conn, func, nullcheck(ci->dsn), nullcheck(ci->username), ci->password ? "xxxxx" : "");
...
inolog("md5 pwd=%s user=%s salt=%02x%02x%02x%02x%02x\n", nullcheck(ci->password), nullcheck(ci->username), (UCHAR)salt[0], (UCHAR)salt[1], (UCHAR)salt[2], (UCHAR)salt[3], (UCHAR)salt[4]);
...
static char CC_initial_log(ConnectionClass *self, const char *func)
{
	...
	qlog("                extra_systable_prefixes='%s', conn_settings='%s' conn_encoding='%s'\n",
		 nullcheck(ci->drivers.extra_systable_prefixes),
		 nullcheck(ci->drivers.conn_settings),
		 encoding ? encoding : "");
	...
	mylog("%s: DSN = '%s', server = '%s', port = '%s', database = '%s', username = '%s', password='%s'\n", func, nullcheck(ci->dsn), nullcheck(ci->server), nullcheck(ci->port), nullcheck(ci->database), nullcheck(ci->username), ci->password ? "xxxxx" : "");
...
inolog("protocol=%s version=%d,%d\n", nullcheck(ci->protocol), self->pg_version_major, self->pg_version_minor);
		if (PROTOCOL_62(ci))
		{
...
CC_connect(ConnectionClass *self, char password_req, char *salt_para)
{
	...
	mylog("sslmode=%s\n", nullcheck(self->connInfo.sslmode));
...
inolog("leng=%d\n", leng);
	...
	while (!ReadyToReturn)
	{
		...
		if ((SOCK_get_errcode(sock) != 0) || (id == EOF))
		{
			...
			mylog("send_query: 'id' - %s\n", nullcheck(CC_get_errormsg(self)));
			CC_on_abort(self, CONN_DEAD);
			ReadyToReturn = TRUE;
			retres = NULL;
			break;
 ...
 */
	...
	if (ci->drivers.conn_settings[0] != '\0')
	{
		...
		while (ptr)
		{
			...
			mylog("%s: result %d, status %d from '%s'\n", func, result, status, nullcheck(ptr));
...
CC_log_error(const char *func, const char *desc, const ConnectionClass *self)
{
	if (self)
	{
		qlog("CONN ERROR: func=%s, desc='%s', errnum=%d, errmsg='%s'\n", nullcheck(func), nullcheck(desc), self->__error_number, nullcheck(self->__error_message));
