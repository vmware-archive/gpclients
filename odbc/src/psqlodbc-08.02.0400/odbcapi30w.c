...
#ifdef DEBUG
//JQ DEBUG TEST FOR AIX ARRAY INSERT
printf("\nSQLSetStmtAttrW=> entered.");
printf("\nSQLSetStmtAttrW=> fAttribute=%d, rgbValue=%p.\n", fAttribute, rgbValue);
#endif // DEBUG
...
#if (defined(WITH_UNIXODBC) || defined(WIN32) ) && ( ! defined(_WIN64) && ! defined(ODBC64) ) 
	SQLPOINTER	pNumAttr
#else
	SQLLEN		*pNumAttr
#endif
...
#ifdef DEBUG
// JQ DEBUG TEST 03-04-2008
	printf("\n\nEntered SQLGetDiagFieldW\n[[%s]] Handle=(%u,%p) Rec=%d Id=%d info=(%p,%d)\n", func, fHandleType,
			handle, iRecord, fDiagField, rgbDiagInfo, cbDiagInfoMax);
printf("\n\n(1)fHandleType = %d, (2)handle = %p, (3)iRecord=%d\n", fHandleType, handle, iRecord);
printf("((4)SQL_DIAG_NUMBER = %d, (5)PTR(NumRecords) = %016X, (6)SIZE_OF_BUFFER = %d\n", (int)fDiagField, rgbDiagInfo, (int)cbDiagInfoMax);
printf("Value of (5)NumRecords = %d, StringLen = %d\n", (int) (*((SQLSMALLINT *)(rgbDiagInfo))), *((SQLSMALLINT *)pcbDiagInfo));
#endif // DEBUG
		...
		default:
			{
				...
				if (bMax <= 0)		// JQ fixed on 03-06-2008 for better reliability
					bMax = sizeof(rgbD);
/* to reproduce the problem for DataDirect
// JQ fixed on 03-04-2008 - this is to fix a core dump caused by the Informatica program
    				char my_rgbDiagInfo[bMax]; 
				memset(my_rgbDiagInfo, '0', bMax);
				ret = PGAPI_GetDiagField(fHandleType, handle, iRecord, fDiagField, my_rgbDiagInfo, bMax, rgbL);
#ifdef DEBUG
printf("\n\nret=%d\n", ret);
printf("\n\nBefore exiting DiagField => cbDiagInfoMax=%d, SQLGetDiagFieldW => my_rgbDiagInfo = %d\n", (int) cbDiagInfoMax, (int) *((SQLINTEGER *)my_rgbDiagInfo));
#endif
				memcpy(rgbD, my_rgbDiagInfo, bMax);
#ifdef DEBUG
printf("Value of (5)NumRecords = %d, StringLen = %d\n", (int) (*((SQLSMALLINT *)rgbD)), *((SQLSMALLINT *)pcbDiagInfo));
#endif
*/
...
#ifdef DEBUG
// JQ DEBUG TEST
printf("\nBefore exiting -> Value of (5)NumRecords = %d\n", (int) (*((SQLSMALLINT *)rgbDiagInfo)));
printf("\n\n exit SQLGetDiagFieldW\n");
#endif
