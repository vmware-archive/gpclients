		...
		UWORD flag)
{
	...
	mylog("%s: status = %d, msg = #%s#\n", func, error->status, nullcheck(msg));
	...
	mylog("	     szSqlState = '%s',len=%d, szError='%s'\n", nullcheck((char *)szSqlState), pcblen, nullcheck((char *)szErrorMsg));
	if (clear_str)
	{
			...
			UWORD flag)
{
	...
	mylog("	     szSqlState = '%s',len=%d, szError='%s'\n", nullcheck((char *)szSqlState), msglen, nullcheck((char *)szErrorMsg));
	if (once_again)
	{
...
{
	if (self)
		qlog("ENVIRON ERROR: func=%s, desc='%s', errnum=%d, errmsg='%s'\n", nullcheck(func), nullcheck(desc), self->errornumber, nullcheck(self->errormsg));
	else
