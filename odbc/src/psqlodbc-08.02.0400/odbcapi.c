...
#ifdef DEBUG
// JQ DEBUG TEST ARRAY INSERT
printf("\n\nEntering %s.\n", func);
PTR Value;
Value = (SQLUINTEGER *) SC_get_IPDF(stmt)->param_processed_ptr;
printf("\n&rows_processed = %p\n", Value);
if (Value != NULL)
	printf("\nrows_processed = %d\n", *((SQLUINTEGER *)Value));
else
	printf("\nrows_processed is a null pointer.\n");
#endif // DEBUG
...
if (Value != NULL)
{
	memcpy((SQLUINTEGER *)Value+1, (SQLUINTEGER *)Value, sizeof(SQLUINTEGER));
	printf("\nrows_processed = %d, place_holder = %d\n", *((SQLUINTEGER *)Value), *((SQLUINTEGER *)Value+1));
}
else
	printf("\nrows_processed is a null pointer.\n");
printf("\n\nExiting %s.\n", func);
