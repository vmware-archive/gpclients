...
/* JQ fixed on 03-14-2008
			stmt->multi_statement is a CHAR, to make a comparison as a value, a cast is mandatory
			((short)(stmt->multi_statement)) 
		if (stmt->multi_statement < 0)
*/
		if ( ((short)(stmt->multi_statement)) < 0) 
			PGAPI_NumParams((StatementClass *) stmt, &num_params);
		if ( ((short)(stmt->multi_statement)) > 0) /* would divide the query into multiple commands and apply V3 parse requests for each of them */
...
RETCODE	Exec_with_parameters_resolved(StatementClass *stmt, BOOL *exec_end)
{
	...
	*exec_end = FALSE;
	conn = SC_get_conn(stmt);
	mylog("%s: copying statement params: trans_status=%d, len=%d, stmt='%s'\n", func, conn->transact_status, strlen(stmt->statement), nullcheck(stmt->statement));
...
inolog("prepare_before_exec=%d srv=%d\n", prepare_before_exec, conn->connInfo.use_server_side_prepare);
	...
	mylog("   stmt_with_params = '%s'\n", nullcheck(stmt->stmt_with_params));
...
#ifdef DEBUG
// JQ DEBUG TEST AIX ARRAY INSERT
printf("\n\n%s: entering...%x\n", func, flag);
#endif
...
// JQ DEBUG TEST AIX ARRAY INSERT 
		if (ipdopts->param_processed_ptr)
		{
printf("\n*ipdopts->param_processed_ptr = %d, ptr = %p\n", *(ipdopts->param_processed_ptr), ipdopts->param_processed_ptr);
...
// JQ DEBUG TEST AIX ARRAY INSERT 
		if (ipdopts->param_processed_ptr)
		{
printf("\njust before return  *ipdopts->param_processed_ptr = %d, ptr = %p\n", *(ipdopts->param_processed_ptr), ipdopts->param_processed_ptr);
		}
printf("\n\n%s: exiting...%x\n", func, flag);
#endif // DEBUG
