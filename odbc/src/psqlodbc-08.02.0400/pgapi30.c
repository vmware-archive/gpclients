...
#ifdef DEBUG
// JQ DEBUG TEST
printf("\n\nPGAPI_GetDiagField => DiagInfoPtr=%p, DiagInfo=%d\n", DiagInfoPtr, *((SQLLEN *)DiagInfoPtr));
#endif
...
#ifdef DEBUG
// JQ DEBUG TEST
printf("\n\nsizeof(DiagInfoPtr) = %d, sizeof(SQLLEN) = %d, sizeof(SQLSMALLINT) = %d\n\n", sizeof(DiagInfoPtr), sizeof(SQLLEN), sizeof(SQLSMALLINT)); 
printf("\n\nExiting PGAPI_GetDiagField => StringLen=%d\n", *((SQLSMALLINT *)StringLengthPtr));
#endif
	return ret;
}
...
#ifdef DEBUG
// JQ DEBUG TEST for AIX ARRARY INSERT
printf("\nPGAPI_GetStmtAttr=> entered.");
printf("\nPGAPI_GetStmtAttr=> Attribute=%d, Value=%p\n", Attribute, Value);
#endif
...
printf("\nPGAPI_GetStmtAttr=>\n\tSQL_ATTR_PARAM_STATUS_PTR, Value(&rows_processed)=%p\n", Value);
#endif
			len = sizeof(SQLPOINTER);
			break;
...
#ifdef DEBUG
// JQ DEBUG TEST for AIX ARRARY INSERT
printf("\nPGAPI_SetStmtAttr=> entered.");
printf("\nPGAPI_SetStmtAttr=> Attribute=%d, Value=%p\n", Attribute, Value);
#endif
