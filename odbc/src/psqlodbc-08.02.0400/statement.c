...
SC_log_error(const char *func, const char *desc, const StatementClass *self)
{
	...
	if (self)
	{
		...
		else
		{
			...
			qlog("%s: func=%s, desc='%s', errnum=%d, errmsg='%s'\n",head, nullcheck(func), nullcheck(desc), self->__error_number, nullcheck(self->__error_message));
		}
...
QResultClass *SendSyncAndReceive(StatementClass *stmt, QResultClass *res, const char *comment)
{
	...
	{
		id = SOCK_get_id(sock);
/* JQ needs to review this statement     warning: comparison is always false due to limited range of data type  */
/* The solution is to include -fsigned-char option in gcc on AIX */
		if ((SOCK_get_errcode(sock) != 0) || (id == EOF))
		{
			SC_set_error(stmt, CONNECTION_NO_RESPONSE, "No response rom the backend", func);

			mylog("%s: 'id' - %s\n", func, nullcheck(SC_get_errormsg(stmt)));
			CC_on_abort(conn, CONN_DEAD);
...
SendParseRequest(StatementClass *stmt, const char *plan_name, const char *query, Int4 qlen, Int2 num_params)
{
	...
	mylog("%s: plan_name=%s query=%s\n", func, nullcheck(plan_name), nullcheck(query));
...
SendExecuteRequest(StatementClass *stmt, const char *plan_name, UInt4 count)
{
	...
	mylog("%s: plan_name=%s count=%d\n", func, nullcheck(plan_name), count);
