...
mylog("CONVERT_FUNCTIONS=" FORMAT_ULEN "\n", value);
	...
	mylog("%s: p='%s', len=%d, value=%d, cbMax=%d\n", func, nullcheck(p), len, value, cbInfoValueMax);
	...
	while (SQL_SUCCEEDED(result))
	{
		...
		if (!atoi(ci->show_system_tables))
		{
			...
			else
			{
				...
				while (prefix[i])
				{
					mylog("table_name='%s', prefix[%d]='%s'\n", nullcheck(table_name), i, nullcheck(prefix[i]));
...
mylog(" and the data=%s\n", attdef);
		...
		qlog("%s: table='%s',field_name='%s',type=%d,name='%s'\n",
			 func, nullcheck(table_name), nullcheck(field_name), field_type, nullcheck(field_type_name));
			...
			for (i = 1; i <= colcnt; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						mylog("%s: column name = '%s'\n", func, nullcheck(column_names[matchidx].col_name));
					}
				}
	...
	if (fk_table_needed && fk_table_needed[0] != '\0')
	{
		...
		while (result == SQL_SUCCESS)
		{
			...
			for (k = 0; k < num_keys; k++)
			{
				...
				mylog("%s: pkey_ptr='%s', pkey='%s'\n", func, nullcheck(pkey_text), nullcheck(pkey));
			...
			for (k = 0; k < num_keys; k++)
			{
				...
				mylog("%s: pk_table = '%s', pkey_ptr = '%s'\n", func, nullcheck(pk_table_fetched), nullcheck(pkey_text));
				...
				mylog("%s: fk_table_needed = '%s', fkey_ptr = '%s'\n", func, nullcheck(fk_table_needed), nullcheck(fkey_text));
				...
				mylog("%s: upd_rule_type = '%i', del_rule_type = '%i'\n, trig_name = '%s'", func, upd_rule_type, del_rule_type, nullcheck(trig_args));
	...
	else if (pk_table_needed[0] != '\0')
	{
		...
		while (result == SQL_SUCCESS)
		{
			...
			for (k = 0; k < num_keys; k++)
			{
				...
				mylog("pkey_ptr = '%s', fk_table = '%s', fkey_ptr = '%s'\n", nullcheck(pkey_text), nullcheck(fk_table_fetched), nullcheck(fkey_text));

				tuple = QR_AddNew(res);

				mylog("pk_table_needed = '%s', pkey_ptr = '%s'\n", nullcheck(pk_table_needed), nullcheck(pkey_text));
				set_tuplefield_string(&tuple[FKS_PKTABLE_CAT], CurrCat(conn));
				...
				mylog("fk_table = '%s', fkey_ptr = '%s'\n", nullcheck(fk_table_fetched), nullcheck(fkey_text));
				set_tuplefield_null(&tuple[FKS_FKTABLE_CAT]);
