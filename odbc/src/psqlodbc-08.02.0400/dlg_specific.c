...
copyAttributes(ConnInfo *ci, const char *attribute, const char *value)
{
	...
	mylog("%s: DSN='%s',server='%s',dbase='%s',user='%s',passwd='%s',port='%s',onlyread='%s',protocol='%s',conn_settings='%s',disallow_premature=%d)\n", func, nullcheck(ci->dsn), nullcheck(ci->server), nullcheck(ci->database), nullcheck(ci->username), ci->password ? "xxxxx" : "", nullcheck(ci->port), nullcheck(ci->onlyread), nullcheck(ci->protocol), nullcheck(ci->conn_settings), ci->disallow_premature);
 ...
 */
	...
	qlog("DSN info: DSN='%s',server='%s',port='%s',dbase='%s',user='%s',passwd='%s'\n",
		 ...
		 nullcheck(ci->server),
		 nullcheck(ci->port),
		 nullcheck(ci->database),
		 nullcheck(ci->username),
		 ci->password ? "xxxxx" : "");
	qlog("          onlyread='%s',protocol='%s',showoid='%s',fakeoidindex='%s',showsystable='%s'\n",
		 nullcheck(ci->onlyread),
		 nullcheck(ci->protocol),
		 nullcheck(ci->show_oid_column),
		 nullcheck(ci->fake_oid_index),
		 nullcheck(ci->show_system_tables));

	if (get_qlog())
	{
		UCHAR	*enc = check_client_encoding(ci->conn_settings);
/* JQ  fixed on 02-27-2008 : cast enc to a string, this fix should be good for both 32bit and 64bit */
		qlog("          conn_settings='%s',conn_encoding='%s'\n",
		 	nullcheck(ci->conn_settings), enc ? ((char *)enc) : "(null)");

		if (enc)
			free(enc);
		qlog("          translation_dll='%s',translation_option='%s'\n",
		 	nullcheck(ci->translation_dll),
		 	nullcheck(ci->translation_option));
	}
}
/*
...
getCommonDefaults(const char *section, const char *filename, ConnInfo *ci)
{
	...
	mylog("globals.extra_systable_prefixes = '%s'\n", nullcheck(comval->extra_systable_prefixes));
